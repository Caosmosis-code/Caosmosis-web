...
public class ComentarioController {
        ...
        @PreAuthorize("isAuthenticated()")
        public ResponseEntity<Void> eliminar(@PathVariable Long articuloId,
                ...
                boolean esAutor = comentario.getUsuario().getId().equals(actual.getId());
                boolean esAdmin = actual.getRol() == Rol.ADMIN;

                if (!esAutor && !esAdmin) {
                        throw new org.springframework.security.access.AccessDeniedException(
                                        "No podés eliminar este comentario");
                }

                eliminarConRespuestas(comentario);

                return ResponseEntity.noContent().build();
        }

        private void eliminarConRespuestas(Comentario comentario) {
                List<Comentario> hijos = comentarioRepository.findByComentarioPadreId(comentario.getId());
                for (Comentario hijo : hijos) {
                        eliminarConRespuestas(hijo);
                }
                likeRepository.deleteByComentarioId(comentario.getId());
        ...
        public ResponseEntity<Map<String, Object>> darLike(@PathVariable Long articuloId,
                ...
                boolean yaLikeado = likeRepository.findByUsuarioIdAndComentarioId(usuario.getId(), comentarioId)
                                .isPresent();
        ...
        private Set<Long> obtenerIdsConLikeDelUsuarioActual(List<Comentario> comentarios,
                        Authentication authentication) {
